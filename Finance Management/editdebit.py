...
def Search():
    ...
    sql = "SELECT id,name,value,reason FROM debit where id = %s"
    val = (debitid,)
...
def Update():
    ...
    try:
       sql = "Update  debit set name= %s,value= %s,reason= %s where id= %s"
...
def Delete():
    ...
    try:
       sql = "delete from debit where id = %s"
       val = (debitid,)
       mycursor.execute(sql, val)
       mysqldb.commit()
       lastid = mycursor.lastrowid
...
def run(): 
    ...
    l.pack()
    root.title("Edit/Delete a Debit Transaction")
